...
async def track_webpage(client: Client, message: Message):
    try:
        ...
        tracked_webpages[url] = {'last_content': last_content, 'frequency': frequency, 'last_checked': time.time()}
...
async def restart_tracking(client: Client, message: Message):
    ...
    await track_webpage(client, message)
...
#===============================================================
    if raw_text5 == "df":
        ...
        raw_text2 = "720"
        res = "1280x720"
        CR = '<a href="http://t.me/UIHASH">𓆩 𝙇𝙚𝙩𝙨 𝙃𝙚𝙡𝙥 𓆪</a>'
        raw_text4 = "df"
        thumb = "no"
...
#===============================================================
            ...
            if "drive" in url:
                try:
                    ...
                    if accept_logs == 1:  
                        ...
                        await bot.send_document(chat_id=-1002494437539, document=file_id, caption=cc1)
                    count+=1
                    os.remove(ka)
                    time.sleep(1)
                ...
                try:
                    ...
                    #if accept_logs == 1:  
                        #file_id = message.document.file_id
                        #await bot.send_document(chat_id=-1002494437539, document=file_id, caption=cc2)
                    count += 1
                ...
                try:
                    ...
                    if accept_logs == 1:  
                        file_id = message.document.file_id
                        await bot.send_document(chat_id=-1002494437539, document=file_id, caption=cc3)
                    count += 1
                    os.remove(f'{name}.{ext}')
                except FloodWait as e:
                    await m.reply_text(str(e))
                ...
                if overlay is not None: 
                    ...
                    if accept_logs == 1:
                        await helper.send_vid(bot, m, url, cc, filename, thumb, name, -1002494437539) 
                    else:
    ...
    if accept_logs == 1:
        await bot.send_message(-1002494437539, f"{end_message}")
...
4. **/Upload** - 💡 Type this before sending your **📃.txt** file.
...
12. **/Upload** - 💡 Type this before sending your **📃.txt** file.
...
BUTTONS = InlineKeyboardMarkup([[InlineKeyboardButton(text="Send Here", url=f"https://t.me/UIHASH")]])
